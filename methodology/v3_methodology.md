...
The cost is reduced pool sizes (~370 → ~180-200), which mechanically increases z-score noise. Validation shows v3 YoY repeatability is ~2-3 points lower than v2's. We accept this trade-off — see Validation in README.
...
**YoY repeatability:** v3 r = 0.85-0.87 across three RS-to-RS pairs, ~2-3 points below v2's 0.88-0.89. Still well above public benchmarks (Corsi/Fenwick ~0.6-0.7, Points/60 ~0.5-0.6).
...
**Playoff vs RS rate inflation:** 1.43-1.64× across four seasons; hits ~1.84×, physical minors ~2.02×, fighting ~0.47×. Stable signature, robust across years.
